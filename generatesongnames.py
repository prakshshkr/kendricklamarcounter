...
class PyLyrics:
	...
	def getAlbums(singer):
		...
		s = BeautifulSoup(requests.get('http://lyrics.wikia.com/{0}'.format(singer)).text, features="html5lib")
...
def main():
	albums = PyLyrics.getAlbums('Kendrick Lamar')
	f = open("name", "w")
	f.write('')
	f.close()
	for album in albums:
		...
		for song in tracks:
			str=song.__repr__().decode("utf-8")
			f = open("name", "a")
			f.write(str)
			f.write('\n')
			f.close()
